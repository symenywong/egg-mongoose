...
function loadModelToApp(app) {
  let customModelDir = app.config.mongoose.baseDir;

  const dir = customModelDir ? path.join(app.config.baseDir, customModelDir) : path.join(app.config.baseDir, 'app/model');
